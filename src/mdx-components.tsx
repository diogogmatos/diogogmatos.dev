...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    ...
    h3: ({ children }) => (
      <h3 className="text-lg font-semibold mt-6 mb-4 scroll-mt-20">
        {children}
      </h3>
    ),
    p: (props) => (
    ...
    a: ({ children, href }) => (
      <Link
        href={href ?? "/"}
        className="text-sm hover:underline cursor-pointer font-medium px-1 py-0.5 rounded-md transition-colors duration-200 bg-white/10 backdrop-blur-md"
        target="_blank"
    ...
    img: (props) => (
      <figure className="mdx-img-figure my-6">
        <ExpandableImage props={{ ...(props as ImageProps) }} />
      </figure>
    ),
    ul: ({ children }) => (
